...
<style>
  ...
  pre { background: #f7fafc; border: 1px solid #e2e8f0; border-radius: 4px; padding: 12px; font-size: 9px; line-height: 1.4; overflow-x: auto; page-break-inside: avoid; }
...
|                         |                                |
|  +---------------------------------------------------+  |
|  |           BACK OFFICE (Flexcube)                   |  |
|  |                                                    |  |
|  |  +----------+  +---------+  +----------+          |  |
|  |  |    GL    |  | Settle- |  |  SWIFT   |          |  |
|  |  | Posting  |  |  ment   |  | Message  |          |  |
|  |  +----------+  +---------+  +----------+          |  |
|  +---------------------------------------------------+  |
|                                                         |
+=========================================================+
...
#### Màn hình Position Dashboard

```
+-------------------------------------------------------+
| POSITION DASHBOARD              31/03/2026 15:30:00   |
+-------------------------------------------------------+
| Currency | Long       | Short      | Net Position     |
|----------|------------|------------|------------------|
| USD      | 50,000,000 | 45,000,000 | +5,000,000       |
| EUR      | 10,000,000 | 12,000,000 | -2,000,000       |
| JPY      | 500,000,000| 480,000,000| +20,000,000      |
| SGD      |  5,000,000 |  3,000,000 | +2,000,000       |
+-------------------------------------------------------+
| [Refresh] [By Product] [By Dealer] [Export]            |
+-------------------------------------------------------+
```
...
+--------+---------+     +------------------+
    ...
    [Settlement]
         ...
         v
+------------------+     +------------------+
| BACK OFFICE      |     | NHNN Reporting   |
| (Flexcube)       |---->| Trạng thái NTE   |
| Settlement &     |     | Doanh số FX      |
| Accounting       |     +------------------+
+------------------+
```
...
```
  06:00   08:00   09:00         15:00    16:00   17:00
    |       |       |             |        |       |
    v       v       v             v        v       v
 +------+ +------+ +----------+ +------+ +------+ +-----+
 |System| |Market| |Trading   | |EOD   | |NHNN  | |EOD  |
 |Start | |Open  | |Session   | |Cut-  | |Report| |Batch|
 |Check | |Data  | |FX/MM/Bond| |off   | |Send  | |Run  |
 +------+ +------+ +----------+ +------+ +------+ +-----+
    |       |       |             |        |       |
    v       v       v             v        v       v
 Config   Rate    Deal Entry   P&L Calc  Auto    GL Post
 Load     Feed    Limit Check  Position  Generate Settle
 Health   Yield   Approval     MTM       Submit   SWIFT
 Check    Curve   Settlement   Reports           Confirm
```
...
```
+--------------------------------------------------+
|           FX SPOT DEAL PROCESS FLOW               |
+--------------------------------------------------+

[Start] --> (1) Nhận yêu cầu FX
                 |
                 v
            (2) Kiểm tra Market Rate
                 |  (Reuters/Bloomberg)
                 v
            (3) Dealer nhập Deal
         ...
         <= $1M   $1-5M  > $5M
            |         |    |
          Auto    Senior  Head of
          Approve Dealer  Treasury
            |     Approve Approve
            |         |    |
            v         v    v
            (7) Deal Status = APPROVED
                 |
                 v
            (8) Gửi Confirmation
                 |  → Đối tác (Email/SWIFT)
           ...
           (10) Settlement (T+2)
                 ...
                 |  - Payment instruction
                 v
           (11) Confirmation nhận tiền
                 |
                 v
           (12) Deal Status = SETTLED
                 |
                 v
              [End]
```
...
| | Chi tiết |
|---|---------|
| **Input** | Nhu cầu thanh khoản, lãi suất thị trường liên ngân hàng |
| **Output** | Deal ticket, GL entries, SWIFT MT320, Xác nhận |
...
```
+--------------------------------------------------+
|        MONEY MARKET DEAL PROCESS FLOW             |
+--------------------------------------------------+

[Start] --> (1) Xác định nhu cầu thanh khoản
                 |  (Thừa → Gửi, Thiếu → Vay)
                 v
            (2) Khảo sát lãi suất thị trường
                 |  - VNIBOR reference
                 |  - Đối tác quoting
                 v
            (3) Dealer liên hệ đối tác
                 |  - Thương lượng rate & tenor
                 v
            (4) Nhập Deal vào hệ thống
                 |  - Loại: Deposit/Borrowing/Repo
                 |  - Amount, Rate, Start/End Date
                 v
            (5) System validate
                 |
            +----+----+
           ...
           OK       FAIL --> Sửa & thử lại
            ...
            +----+----+
            |         |
           OK      BREACH --> Alert + Block
            |
            v
            (7) Phê duyệt theo ngưỡng
                 |
            +----+----+----+
            |         |    |
         <= 50B   50-100B  > 100B
          ...
          Dealer   Treasury
           ...
           (11) Settlement
                 ...
                 v
           (12) Theo dõi đáo hạn
                 |  - Tính lãi accrued daily
                 |  - Alert trước đáo hạn 2 ngày
                 v
           (13) Đáo hạn / Rollover
                 |
            +----+----+
...
| | Chi tiết |
|---|---------|
| **Input** | Chiến lược đầu tư, giá thị trường, yield curve |
| **Output** | Deal ticket, lệnh mua/bán, GL entries, báo cáo danh mục |
...
[Start] --> (1) Phân tích thị trường
         ...
         bạc NN
           ...
           (10) Flexcube Booking
                 ...
                 |  - Accrual setup
                 v
           (11) Settlement (T+1 hoặc T+2)
                 |  - Thanh toán qua VSD
                 v
           (12) Theo dõi danh mục
                 |  - Coupon payment
                 |  - MTM daily
                 |  - Maturity tracking
                 v
...
| Actor | Vai trò |
|-------|---------|
| **System (Auto)** | Chạy batch tính toán EOD |
| **Risk Manager** | Rà soát kết quả, báo cáo |
| **Head of Treasury** | Xem P&L, ra quyết định |
| **Back Office** | Đối chiếu, xác nhận số liệu |

### Input / Output

| | Chi tiết |
|---|---------|
| **Input** | Tất cả deal trong ngày, Market closing rates, Yield curves |
| **Output** | Position report, P&L report (daily/MTD/YTD), Risk report |
...
```
+--------------------------------------------------+
|       POSITION & P&L DAILY PROCESS FLOW           |
+--------------------------------------------------+

[15:00 - Market Close]
         |
         v
    (1) System capture closing rates
         |  - FX closing rates
         |  - Yield curves
         |  - Bond prices
         v
    ...
    (3) Mark-to-Market
         ...
         v
    (4) P&L Calculation
         |  - Realized P&L (closed deals)
         |  - Unrealized P&L (open deals)
         |  - Accrued interest (MM/Bonds)
         |  - Total P&L = Sum all
         v
    (5) P&L Attribution
         |  - Rate effect
         ...
         |  - Spread effect
         v
    (6) Limit utilization update
         |  - Recalculate all limits
         |  - Update usage %
         v
    (7) Risk metrics update
         |  - VaR calculation
         |  - Stress test run
         v
    (8) Report generation
...
| Actor | Vai trò |
|-------|---------|
| **System** | Phát hiện breach, gửi cảnh báo |
| **Dealer** | Nhận cảnh báo, điều chỉnh vị thế |
| **Risk Manager** | Đánh giá, quyết định xử lý |
| **Head of Treasury** | Phê duyệt override hoặc chỉ đạo |
| **Risk Committee** | Xử lý breach nghiêm trọng |

### Input / Output

| | Chi tiết |
|---|---------|
| **Input** | Deal mới hoặc thay đổi giá thị trường |
| **Output** | Alert, block, override record, breach report |
...
[Trigger: Deal Entry / Market Move]
         |
         v
    (1) System tính utilization
    ...
    +----+----+----+----+----+
    |    |    |    |    |    |
  0-70% 70-85 85-95 95-100 >100%
    |    |    |    |    |
  Normal Warn Alert Crit Breach
    |    |    |    |    |
    v    v    v    v    v
   OK  (3a) (3b) (3c) (3d)
    |  Yellow Red  Block Hard
    |  Alert  Alert Deal  Block
    |    |    |    |    |
    |    v    v    v    v
    |  Dealer Senior Head  Risk
    |  notif  Dealer Treas Comm.
    |    |    |    |    |
    |    |    |    v    v
    |    |    | (4) Override?
    |    |    |    |
    ...
    |    |    | v
    v    v    v v
    (7) Deal tiếp tục xử lý
         |
         v
    (8) Breach Report tự động
         |  - Gửi Risk Committee
         |  - Lưu audit trail
         v
    (9) Action plan trong 24h
         |  - Giảm vị thế
         |  - Tăng hạn mức (cần phê duyệt)
         |  - Hedge
         v
   (10) Theo dõi regularize
         |  System check mỗi giờ
         |
    +----+----+
    |         |
  Resolved  Still breach
    |         |
    v         v
  [End]  Escalate tiếp
         → Management
...
| Actor | Vai trò |
|-------|---------|
| **System** | Tự động tạo báo cáo |
| **Back Office** | Kiểm tra, xác nhận dữ liệu |
| **Compliance** | Rà soát tuân thủ |
| **Head of Treasury** | Phê duyệt trước khi gửi |
| **IT** | Hỗ trợ kỹ thuật truyền file |

### Input / Output

| | Chi tiết |
|---|---------|
| **Input** | Deal data, position, rates, GL balances |
| **Output** | File báo cáo (XML/Excel), confirmation gửi NHNN |
...
```
+--------------------------------------------------+
|          NHNN REPORTING PROCESS FLOW              |
+--------------------------------------------------+

[15:00 - EOD Trigger]
         |
         v
    (1) System tự động tạo báo cáo
         |  - Query deal data
         |  - Calculate positions
   ...
   Pass     Fail
    |         |
    |    (2a) Alert Back Office
    |         |  Sửa dữ liệu
    |         |  Re-generate
    |         |
    v         v
    (3) Back Office review
         |  - Kiểm tra số liệu
         |  - So sánh với ngày trước
         |  - Xác nhận chính xác
         v
    (4) Compliance review
         |  - Kiểm tra tuân thủ format
         |  - Kiểm tra regulatory rules
         v
    (5) Head of Treasury phê duyệt
         |
    +----+----+
    |         |
  Approve   Reject
    ...
    v
    (6) Gửi báo cáo NHNN
         |  - Qua hệ thống NHNN online
         |  - Hoặc secure email
         v
    (7) Nhận xác nhận
         |  - Confirmation từ NHNN
         v
    (8) Lưu trữ
         |  - Archive báo cáo + confirmation
         |  - Audit trail
         v
    [End]
```
