...
**Concurrency:** Use `SELECT ... FOR UPDATE` or check `version` on update to prevent overselling
...
**⚠️ ACT_ACT Day Count:** Application must dynamically determine year basis (365 or 366) by checking if the accrual period spans Feb 29. This is NOT stored in DB.
...
| `effective_to` | DATE NULL | Effective end date (NULL = current active) |
...
**SCD Type 2:** History preserved. When limit changes, set `effective_to` on old row + `is_current = false`, insert new row with `is_current = true`
...
| 8 | Auth | `user_sessions` | Session management (standalone mode) |
...
| 16 | Master | `business_calendar` | Business day calendar |
...
| 27 | Workflow | `deal_sequences` | Gapless deal number generation |
| 28 | Workflow | `approval_actions` | Approval/rejection history |
| 29 | Workflow | `status_transition_rules` | State machine config |
...
### Why `approval_actions` AND `audit_logs`?
- `approval_actions` = **workflow engine query** — find pending items, track approval chain per deal, drive UI state
- `audit_logs` = **compliance/regulatory** — complete immutable history, includes non-approval events (edits, logins, exports)
- Different query patterns, different retention policies
- Not duplicate — they serve fundamentally different consumers

### Why `deal_sequences`?
- Banking requires **gapless sequential numbers** — regulators and auditors expect no gaps
- `BIGSERIAL` has gaps on rollback, transaction abort, or crash recovery
- `deal_sequences` + advisory lock = **guaranteed gapless** per module per day
- Pattern: `SELECT ... FOR UPDATE` → increment `last_sequence` → format as `FX-20260403-0001`

### Why `business_calendar`?
- Settlement dates **must** be business days — T+1, T+2 calculations need to skip holidays
- Exchange rate lookups need "previous business day" — not just "yesterday"
- Without this table, application guesses weekends only and **misses holidays** (Tết, 30/4, 2/9, etc.)
- Pre-loaded yearly — simple admin maintenance
