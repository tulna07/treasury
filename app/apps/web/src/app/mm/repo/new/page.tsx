...
export default function MMRepoNewPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate(payload, {
      onSuccess: (res) => {
        toast.success(t("mm.repo.create.success"));
        router.push(`/mm/repo/${res.id}`);
      },
      onError: (err) => {
    ...
    <>
      ...
      <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
        <div className="flex items-center gap-4 pt-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">{t("mm.repo.create.title")}</h1>
            <p className="text-muted-foreground">{t("mm.repo.create.description")}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle>{t("mm.repo.create.dealInfo")}</CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label>{t("mm.repo.create.sessionName")} *</Label>
                <Input
                  value={form.session_name}
                  onChange={(e) => set("session_name", e.target.value)}
                  placeholder={t("mm.repo.create.sessionNamePlaceholder")}
                  required
                />
              </div>

              <div className="space-y-2">
                <Label>{t("mm.repo.create.counterparty")} *</Label>
                <Select value={form.counterparty_id} onValueChange={(v) => v && set("counterparty_id", v)}>
                  <SelectTrigger><SelectValue placeholder={t("mm.repo.create.selectCounterparty")}>{(v: string) => { const cp = counterparties.find((c) => c.id === v); return cp ? `${cp.code} — ${cp.name}` : v; }}</SelectValue></SelectTrigger>
                  <SelectContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle>{t("mm.repo.create.bondInfo")}</CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-2 md:col-span-2">
                <Label>{t("mm.repo.create.selectBond")} *</Label>
                <Popover open={bondOpen} onOpenChange={setBondOpen}>
                  ...
                  <PopoverContent className="w-[var(--anchor-width)] p-0" align="start">
                    <Command>
                      <CommandInput placeholder={t("mm.repo.create.selectBond")} />
                      <CommandList>
          ...
          <Card>
            <CardHeader>
              <CardTitle>{t("mm.repo.create.financials")}</CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label>{t("mm.repo.create.notionalAmount")} *</Label>
                <Input
                  type="number"
                  min="0"
                  step="any"
              ...
              <div className="space-y-2">
                <Label>{t("mm.repo.create.winningRate")} *</Label>
                <Input
                  type="number"
                  min="0"
                  step="any"
              ...
              <div className="space-y-2">
                <Label>{t("mm.repo.create.haircutPct")} *</Label>
                <Input
                  type="number"
                  min="0"
                  step="any"
          ...
          <Card>
            <CardHeader>
              <CardTitle>{t("mm.repo.create.dates")}</CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
              <div className="space-y-2">
                <Label>{t("mm.repo.detail.tradeDate")} *</Label>
                <DatePicker
              ...
              <div className="space-y-2">
                <Label>{t("mm.repo.create.tenorDays")} *</Label>
                <Input
                  type="number"
                  min="1"
                  step="1"
              ...
              <div className="space-y-2">
                <Label>{t("mm.repo.create.settlementDate1")} *</Label>
                <DatePicker
              ...
              <div className="space-y-2">
                <Label>{t("mm.repo.create.settlementDate2")} *</Label>
                <DatePicker
          ...
          <Card>
            <CardHeader>
              <CardTitle>{t("mm.repo.create.note")}</CardTitle>
            </CardHeader>
            <CardContent>
              <Textarea
                value={form.note}
                onChange={(e) => set("note", e.target.value)}
                placeholder={t("mm.repo.create.notePlaceholder")}
                rows={3}
              />
            </CardContent>
          </Card>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="submit" disabled={createMutation.isPending}>
              {createMutation.isPending ? t("common.saving") : t("mm.repo.create.submit")}
            </Button>
          </div>
        </form>
      </div>
