...
import { useMMOMODeal, useApproveMMOMO, useMMOMOHistory } from "@/hooks/use-mm";
...
function AttachmentsPanel({ dealId, canUpload }: { dealId: string; canUpload: boolean }) {
  ...
  const { data: attachments = [], isLoading } = useAttachments("MM_OMO", dealId);
  ...
  const handleUpload = useCallback((files: FileList | null) => {
    ...
    uploadMutation.mutate(
      { dealModule: "MM_OMO", dealId, files: Array.from(files) },
      {
...
export default function MMOMODetailPage() {
  const { t } = useTranslation();
  const router = useRouter();
  const params = useParams();
  const id = params.id as string;

  const { data: deal, isLoading, isError, error } = useMMOMODeal(id);
  const approveMutation = useApproveMMOMO(id);
  const { data: historyData } = useMMOMOHistory(id);
        ...
        <div className="flex flex-col gap-4 pt-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">{t("mm.omo.detail.title")}</p>
            </div>
          </div>
          {canApprove && (
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Card>
                <CardHeader className="flex flex-row items-center gap-2 space-y-0 pb-3">
                  ...
                  <CardTitle className="text-base">{t("mm.omo.detail.dealInfo")}</CardTitle>
                </CardHeader>
                <CardContent>
                  <dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                    <InfoRow label={t("mm.omo.detail.dealNumber")}><span className="font-mono">{deal.deal_number}</span></InfoRow>
                    <InfoRow label={t("mm.omo.detail.dealSubtype")}><Badge variant="outline">{deal.deal_subtype}</Badge></InfoRow>
                    <InfoRow label={t("mm.omo.detail.sessionName")}>{deal.session_name}</InfoRow>
                    <InfoRow label={t("mm.omo.detail.counterparty")}>{deal.counterparty_code} — {deal.counterparty_name}</InfoRow>
                    <InfoRow label={t("mm.omo.detail.status")}><MMStatusBadge status={deal.status} /></InfoRow>
                  </dl>
                  {deal.note && (
                    <>
                      <Separator className="my-4" />
                      <div className="text-sm">
                        <span className="text-muted-foreground">{t("mm.omo.detail.note")}:</span> <span>{deal.note}</span>
                      </div>
                    </>
                  )}
                </CardContent>
              ...
              <Card>
                <CardHeader className="flex flex-row items-center gap-2 space-y-0 pb-3">
                  ...
                  <CardTitle className="text-base">{t("mm.omo.detail.bondFinancial")}</CardTitle>
                </CardHeader>
                <CardContent>
                  <dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                    <InfoRow label={t("mm.omo.detail.bondCode")}><span className="font-mono">{deal.bond_code}</span></InfoRow>
                    <InfoRow label={t("mm.omo.detail.bondIssuer")}>{deal.bond_issuer}</InfoRow>
                    <InfoRow label={t("mm.omo.detail.couponRate")}><span className="tabular-nums">{fmtRate(deal.bond_coupon_rate)}%</span></InfoRow>
                    {deal.bond_maturity_date && (
                      <InfoRow label={t("mm.omo.detail.bondMaturityDate")}>{formatDate(deal.bond_maturity_date)}</InfoRow>
                    )}
                    <InfoRow label={t("mm.omo.detail.notionalAmount")}><span className="tabular-nums">{fmtAmt(deal.notional_amount)}</span></InfoRow>
                    <InfoRow label={t("mm.omo.detail.winningRate")}><span className="tabular-nums">{fmtRate(deal.winning_rate)}%</span></InfoRow>
                    <InfoRow label={t("mm.omo.detail.haircutPct")}><span className="tabular-nums">{fmtPct(deal.haircut_pct)}%</span></InfoRow>
                  </dl>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader className="flex flex-row items-center gap-2 space-y-0 pb-3">
                  ...
                  <CardTitle className="text-base">{t("mm.omo.detail.settlementDates")}</CardTitle>
                </CardHeader>
                <CardContent>
                  <dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                    <InfoRow label={t("mm.omo.detail.tradeDate")}>{formatDate(deal.trade_date)}</InfoRow>
                    <InfoRow label={t("mm.omo.detail.tenorDays")}>{deal.tenor_days} {t("mm.detail.days")}</InfoRow>
                    <InfoRow label={t("mm.omo.detail.settlementDate1")}>{formatDate(deal.settlement_date_1)}</InfoRow>
                    <InfoRow label={t("mm.omo.detail.settlementDate2")}>{formatDate(deal.settlement_date_2)}</InfoRow>
                  </dl>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader className="flex flex-row items-center gap-2 space-y-0 pb-3">
                  ...
                  <CardTitle className="text-base">{t("mm.omo.detail.metadata")}</CardTitle>
                </CardHeader>
                <CardContent>
                  <dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                    <InfoRow label={t("mm.omo.detail.createdBy")}>{deal.created_by_name}</InfoRow>
                    <InfoRow label={t("mm.omo.detail.createdAt")}>{formatDate(deal.created_at)}</InfoRow>
                  </dl>
                </CardContent>
              </Card>
            </div>
