...
interface BondAttachmentListProps {
  dealId: string;
  canDelete?: boolean;
  currentUserId?: string;
}

export function BondAttachmentList({
  dealId,
  canDelete = false,
  currentUserId,
}: BondAttachmentListProps) {
  const { t } = useTranslation();
  const { data: attachments, isLoading } = useAttachments("BOND", dealId);
  ...
  const handleDelete = (att: Attachment) => {
    ...
    deleteMutation.mutate(
      { id: att.id, dealModule: "BOND", dealId },
      {
